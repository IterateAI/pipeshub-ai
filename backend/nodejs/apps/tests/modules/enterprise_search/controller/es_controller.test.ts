...
describe('Enterprise Search Controller', () => {
  ...
  describe('chat attachment handlers', () => {
    ...
    describe('uploadChatAttachments', () => {
      ...
      it('should accept a native XLSX attachment', async () => {
        sinon.stub(AIServiceCommand.prototype, 'execute').callsFake(function (this: any) {
          const payload = JSON.parse(this.body)
          expect(payload.attachments[0].fileName).to.equal('report.xlsx')
      ...
      it('should accept a native SQLite attachment', async () => {
        sinon.stub(AIServiceCommand.prototype, 'execute').callsFake(function (this: any) {
          const payload = JSON.parse(this.body)
          expect(payload.attachments[0].fileName).to.equal('analytics.sqlite')
          expect(payload.attachments[0].mimeType).to.equal('application/vnd.sqlite3')
          return Promise.resolve({ statusCode: 200, data: { attachments: [] } } as any)
        })
