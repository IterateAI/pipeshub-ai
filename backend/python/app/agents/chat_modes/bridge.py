...
async def _build_no_tools_messages(
    *, query: str, ai_models_config: dict[str, Any], context_text: str, user_data: str,
    ...
    custom_prompt = ai_models_config.get("customSystemPrompt", "")
    if custom_prompt:
        system_prompt += f"\n\n{custom_prompt}"
    content, ref_mapper = get_message_content(
        [], {}, user_data, query, mode="no_tools",
    ...
    *, query_info: dict[str, Any], user_info: dict[str, Any], llm: "BaseChatModel",
    ...
    try:
        async for stream_event in stream_llm_response_with_tools(
            llm=llm, messages=messages, final_results=prefetch.final_results if prefetch else [],
            all_queries=[query_info.get("query", "")], retrieval_service=retrieval_service,
            user_id=user_id, org_id=org_id,
            virtual_record_id_to_result=prefetch.virtual_record_id_to_result if prefetch else {},
            blob_store=blob_store, is_multimodal_llm=is_multimodal_llm, context_length=context_length,
            tools=[], tool_runtime_kwargs={}, target_words_per_chunk=1, mode="no_tools",
            ref_mapper=ref_mapper, conversation_id=query_info.get("conversationId"),
        ):
    ...
    async def _produce() -> None:
        ...
        try:
            ...
            if prefetch_result is not None and not prefetch_result.is_empty:
                ...
                context.tool_state["citation_ref_mapper"] = prefetch_result.citation_ref_mapper
